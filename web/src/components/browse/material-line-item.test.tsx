...
vi.mock("@/components/home/material-preview", () => ({
  MaterialPreview: ({ material }: { material: { title?: string } }) => (
    <div data-testid="material-preview">{material?.title || "preview"}</div>
  ),
}));
...
describe("MaterialLineItem", () => {
  ...
  it("renders the material title and thumbnail preview", async () => {
    await render(
      <MaterialLineItem material={material} pathBase="/browse/math" navIndex={0} />,
    );
    expect(container.textContent).toContain("Doc 1");
    expect(container.querySelector("[data-testid='material-preview']")).not.toBeNull();
  });
