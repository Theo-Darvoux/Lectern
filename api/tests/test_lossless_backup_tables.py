...
@pytest.mark.asyncio
async def test_allowed_domains_roundtrip(db_session: AsyncSession, tmp_path: Path) -> None:
    domain_id = str(uuid.uuid4())
    zip_path = _make_zip_with_rows(
        tmp_path,
        {
            "allowed_domains": [
                {"id": domain_id, "domain": "example.com", "auto_approve": True, "created_at": NOW}
            ],
        },
    )
    with _no_s3():
    ...
    r = await db_session.execute(
        text("SELECT domain FROM allowed_domains WHERE id = :id"), {"id": domain_id}
    )
...
@pytest.mark.asyncio
async def test_dead_letter_jobs_roundtrip(db_session: AsyncSession, tmp_path: Path) -> None:
    ...
    r = await db_session.execute(
        text("SELECT job_name FROM dead_letter_jobs WHERE id = :id"), {"id": job_id}
    )
...
async def test_notifications_roundtrip(db_session: AsyncSession, tmp_path: Path) -> None:
    ...
    r = await db_session.execute(
        text("SELECT title FROM notifications WHERE id = :id"), {"id": nid}
    )
...
async def test_view_history_roundtrip(db_session: AsyncSession, tmp_path: Path) -> None:
    ...
    r = await db_session.execute(
        text("SELECT COUNT(*) FROM view_history WHERE id = :id"), {"id": vid}
    )
...
async def test_material_likes_roundtrip(db_session: AsyncSession, tmp_path: Path) -> None:
    ...
    r = await db_session.execute(
        text("SELECT COUNT(*) FROM material_likes WHERE id = :id"), {"id": lid}
    )
    assert r.scalar_one() == 1
...
async def test_material_favourites_roundtrip(db_session: AsyncSession, tmp_path: Path) -> None:
    ...
    zip_path = _make_zip_with_rows(
        tmp_path,
        {
            ...
            "materials": [_material_row(mid, uid)],
            "material_favourites": [
                {"id": fid, "user_id": uid, "material_id": mid, "created_at": NOW}
            ],
        },
    )
    with _no_s3():
    ...
    r = await db_session.execute(
        text("SELECT COUNT(*) FROM material_favourites WHERE id = :id"), {"id": fid}
    )
    assert r.scalar_one() == 1
...
async def test_directory_likes_roundtrip(db_session: AsyncSession, tmp_path: Path) -> None:
    ...
    zip_path = _make_zip_with_rows(
        tmp_path,
        {
            ...
            "directory_likes": [
                {"id": lid, "user_id": uid, "directory_id": did, "created_at": NOW}
            ],
        },
    )
    with _no_s3():
    ...
    r = await db_session.execute(
        text("SELECT COUNT(*) FROM directory_likes WHERE id = :id"), {"id": lid}
    )
    assert r.scalar_one() == 1
...
async def test_directory_favourites_roundtrip(db_session: AsyncSession, tmp_path: Path) -> None:
    ...
    zip_path = _make_zip_with_rows(
        tmp_path,
        {
            ...
            "directory_favourites": [
                {"id": fid, "user_id": uid, "directory_id": did, "created_at": NOW}
            ],
        },
    )
    with _no_s3():
    ...
    r = await db_session.execute(
        text("SELECT COUNT(*) FROM directory_favourites WHERE id = :id"), {"id": fid}
    )
    assert r.scalar_one() == 1
...
@pytest.mark.asyncio
async def test_annotations_self_ref_topological(db_session: AsyncSession, tmp_path: Path) -> None:
        ...
        tmp_path,
        {
            ...
            "annotations": [
                # child listed first — topo sort must flip order
                {
                    ...
                    "selection_text": None,
                    "position_data": None,
                    "thread_id": parent_ann,
                    "reply_to_id": parent_ann,
                    "created_at": NOW,
                    "updated_at": NOW,
                },
                {
