...
@pytest.mark.asyncio
async def test_run_thumbnail_stage_qcm() -> None:
    """QCM JSON files should be successfully converted to a WebP thumbnail."""
    import json
    from app.core.processing import ProcessingFile
    from app.workers.upload.stages.thumbnail import run_thumbnail_stage

    # Create a temp QCM file
    qcm_data = {
        "version": 1,
        "chapters": [
            {
                ...
                "questions": [
                    {
                        ...
                        "answers": [
                            {"id": "a1", "text": "Paris", "correct": True},
                            {"id": "a2", "text": "London", "correct": False}
                        ]
                    },
                    {
                        "id": "q2",
                        "text": "What is 2 + 2?",
                        "answers": [
                            {"id": "a3", "text": "4", "correct": True},
                            {"id": "a4", "text": "5", "correct": False}
                        ]
                    }
                ]
            }
    ...
    with tempfile.NamedTemporaryFile(suffix=".qcm", delete=False, mode="w", encoding="utf-8") as f:
    ...
    thumb_path_str = None
    try:
        thumb_path_str = await run_thumbnail_stage(pf, "application/vnd.wikint.qcm+json", "test.qcm")
        assert thumb_path_str is not None
        thumb_path = Path(thumb_path_str)
        assert thumb_path.exists()
        assert thumb_path.suffix == ".webp"
