...
async def compress_file_path(
    ...
    try:
        ...
        if mime_type == "image/svg+xml":
            source_path = await _shielded_to_thread(_optimize_svg_to_path, file_path, filename)

            try:
                compressed = await _shielded_to_thread(_gzip_compress_path, source_path)
        ...
        if mime_type.startswith("audio/"):
            ...
            new_mime = "audio/ogg" if opus_path != file_path else mime_type
            return CompressResultPath(opus_path, opus_path.stat().st_size, None, new_mime)

        if mime_type in ZIP_MIME_TYPES:
            try:
                # Embedded raster images may be materialized up to the per-entry
                # limit, so ZIP recompression shares the global image memory cap.
                async with image_guard():
                    compressed = await _shielded_to_thread(_recompress_zip_path, file_path)
                return CompressResultPath(compressed, compressed.stat().st_size, None, mime_type)
        ...
        if mime_type.startswith("text/") or mime_type in GZIP_MIME_TYPES:
            compressed = await _shielded_to_thread(_gzip_compress_path, file_path)
